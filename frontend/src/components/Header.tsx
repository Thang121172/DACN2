		...
		<header className="bg-white shadow-sm">
			<div className="container flex items-center justify-between py-4">
				...
				<nav className="space-x-4 text-sm text-gray-700">
					...
					{/* role-aware links */}
					{user?.role === 'merchant' && <Link to="/merchant" className="hover:underline">Merchant Dashboard</Link>}
					{user?.role === 'shipper' && <Link to="/shipper" className="hover:underline">Shipper Dashboard</Link>}
					{(!user || user.role === 'customer') && <Link to="/customer" className="hover:underline">Menu</Link>}
